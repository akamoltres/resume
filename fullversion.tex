\documentclass{resume}

\usepackage{setspace}

\geometry{papersize={8.5in,36in}}

\begin{document}

\topname{Abhiram Krishnan}{abhiram.krishnan27@gmail.com}{1 (269) 929 7174}{}{}{}{}

\section{Education}
\basesubsection{University of Michigan - Ann Arbor}
\datedsubsection{MEng Space Engineering (GPA: 4.000)}{Graduated: December 2018}
\datedsubsection{BSE Aerospace Engineering (GPA: 3.464)}{Graduated: May 2017}
Minors in Computer Science and Electrical Engineering
\datedbfsubsection{Kalamazoo Area Math and Science Center}{Graduated: June 2013}
\datedbfsubsection{Portage Central High School (GPA: 4.000)}{Graduated: June 2013}

\section{Work Experience}
\worksubsection{Space Exploration Technologies, Inc.}{Software Engineer (Starlink)}{September 2021 -- Present}
\begin{itemize}
    \item Owned guidance and navigation software across multiple generations of Starlink satellites
    \item Responsible for star tracker, flight computer, time synchronization, and configuration management software
    \item Led flight software bringup for multiple v2 satellite revisions and served as mission director for inaugural launch
    \item Delivered multiple flight software builds to the launch site for final satellite integration
    \item Periodic on-call shifts as the point-of-contact for satellite bus software across the full Starlink fleet
\end{itemize}
\worksubsection{Blue Origin, LLC}{Software Engineer}{April 2020 -- September 2021}
\begin{itemize}
    \item Key contributor to BE-4 rocket engine controller software development and qualification efforts
    \item Technical owner for microcontroller bootloader and self-tests
    \item Rewrote microcontroller peripheral drivers and proposed associated requirement updates
    \item Analyzed impact of requirement changes and implemented software changes
\end{itemize}
\worksubsection{Blue Origin, LLC}{Aerospace Engineer (New Graduate Rotation)}{February 2019 -- April 2020}
\begin{itemize}
    \item Built test software framework for avionics hardware test and trained team on use
    \item Delivered software configurations for acceptance testing of engine controllers
    \item Built up test rack to automate board development testing
    \item Completed preliminary design and analysis for manual fill/drain valve
    \item Built up automation and data processing framework for injector component acceptance testing
    \item Contributed to engine test data processing software
\end{itemize}
\worksubsection{Blue Origin, LLC}{Engineering Intern (New Glenn Flight Autonomy)}{May 2018 -- August 2018}
\begin{itemize}
    \item Developed custom visualization tool for New Glenn GNC team
    \item Delivered physics-based model of New Glenn Stage Two to verify closed loop models
    \item Refreshed analysis on launch tower clearance
    \item Built graphical tool to visualize/modify attitude maneuvers
\end{itemize}
\worksubsection{Graduate Student Instructor}{EECS 592, Artificial Intelligence}{January 2018 -- April 2018}
\begin{itemize}
    \item Held office hours to help students with homework and exam preparation
    \item Wrote homework solutions and graded exams
\end{itemize}
\worksubsection{Space Exploration Technologies, Inc}{Associate Engineer (Mechanical Design)}{August 2017 -- December 2017}
\begin{itemize}
    \item Verified secondary retention properties of threaded fasteners
    \item Characterized electrical bonding capabilities of small screws
    \item Designed and manufactured helicopter mount for antenna testing
    \item Developed laser welding workflow for hermetically sealing avionics boxes
    \item Researched measurement techniques for bolt preload thermal testing
\end{itemize}
\worksubsection{Blue Origin, LLC}{Engineering Intern (Avionics Software)}{May 2017 -- August 2017}
\begin{itemize}
    \item Ported software from real time microcontroller to Linux
    \item Evaluated architectural decisions for original version of software
    \item Wrote code to communicate with peripherals
\end{itemize}
\worksubsection{Boeing Defense, Space, and Security}{Flight Engineering Intern}{May 2016 -- August 2016}
\begin{itemize}
    \item Applied TVC hydraulic model to validate actuators against step response requirement
    \item Updated and validated Simulink model of TVC actuators based on new system parameters
    \item Resolved discrepancies between system performance table and CAD model of subsystem
\end{itemize}
\worksubsection{Raytheon Missile Systems}{GNC Engineering Intern}{June 2015 -- August 2015}
\begin{itemize}
    \item Performed trade study on rocket motors, updating relevant factors and providing range data
    \item Completed aerodynamic and structural analysis and updated 3DOF simulation for wind tunnel test
    \item Implemented target update models and updated simulation parameters in preparation for flight test
    \item Designed and implemented requirements tests for missile simulation framework
\end{itemize}
\worksubsection{QARDS Code, LLC}{Co-Founder/iOS Developer}{June 2012 -- August 2014}
\begin{itemize}
    \item Helped develop Report It! iOS app for local residents to report city problems
\end{itemize}
\datedbfsubsection{BlueGranite, Inc.}{June 2011 -- August 2013}
\dateditsubsubsection{Data Mining Intern}{November 2012 -- August 2013}
\begin{itemize}
    \item Competed as a team in Kaggle data mining competitions
    \item Provided publicity for the BlueGranite data mining business
    \item Learned R in the context of data mining
\end{itemize}
\dateditsubsubsection{iOS Intern}{June 2011 -- March 2012}
\begin{itemize}
    \item Built iOS app, BlueGranite Nitro, in a team of five students
    \item Learned Objective-C and iOS programming
\end{itemize}

\section{Activities}
\datedbfsubsection{Michigan Aeronautical Science Association (MASA)}{September 2013 -- December 2018}
\dateditsubsubsection{Avionics/Structures Team Member}{January 2018 -- December 2018}
\begin{itemize}
    \item Led liquid oxygen tank run tank design and manufacturing
    \item Developed cryogenic seal calculator
    \item Managed ground software development
    \item Served as technical and professional mentor for younger students
\end{itemize}
\dateditsubsubsection{Business Manager/Mentor}{June 2016 -- June 2017}
\begin{itemize}
    \item Managed supply chain and industry sponsors for student rocket team
    \item Mentored new team members in design process for hybrid rocket upgrades and active roll control
\end{itemize}
\dateditsubsubsection{Chief Engineer}{June 2015 -- June 2016}
\begin{itemize}
    \item Team lead for design and flight of student built hybrid rocket, placing 2nd at IREC 2016
    \item Completed test launch prior to end of academic year for the first time in recent history
    \item Designed and machined components for airframe, propulsion, and avionics integration
\end{itemize}
\dateditsubsubsection{Safety Officer}{June 2014 -- June 2015}
\begin{itemize}
    \item Wrote and enforced standard operating procedures and safety protocols for rocket motor tests
    \item Built ground support electronics for data acquisition during rocket motor tests
    \item Designed rocket fins and attachment mechanism using SolidWorks
\end{itemize}
\dateditsubsubsection{Aerodynamics Team}{September 2013 -- May 2014}
\begin{itemize}
    \item Designed rocket and built component CAD models
    \item Ran CFD analyses on rocket to confirm stability in flight
    \item Optimized MATLAB rocket flight simulator
\end{itemize}
\datedbfsubsection{American Institute of Aeronautics/Astronautics (AIAA)}{September 2014 -- Present}
\dateditsubsubsection{Pacific Northwest Section, Secretary Elect}{July 2019 -- Present}
\dateditsubsubsection{U-M AIAA Student Chapter, VP - Events}{January 2018 -- April 2018}
\dateditsubsubsection{U-M AIAA Student Chapter, Professional Development Chair}{June 2016 -- June 2017}
\begin{itemize}
    \item Led a committee to organize events for student development and professional networking
    \item Developed younger members' networking and event planning skills
\end{itemize}
\dateditsubsubsection{U-M AIAA Student Chapter, VP - Events}{June 2015 -- June 2016}
\begin{itemize}
    \item Organized tours, networking events, and seminars for AIAA students
    \item Planned and executed 20 student trip to California for SciTech 2016
\end{itemize}
\datedbfsubsection{U-Michigan Competitive Programming Team (ACM)}{September 2013 -- November 2014}
\begin{itemize}
    \item Second place, Grand Valley site
\end{itemize}
\datedbfsubsection{KAMSC Competitive Programming Team}{September 2010 -- June 2013}

\section{Projects}
\worksubsection{SPACE582 Spacecraft Design Course}{EPS Lead}{September 2018 -- December 2018}
\begin{itemize}
    \item Wrote requirements for and designed electrical power system for Mars orbiter
\end{itemize}
\worksubsection{SPACE584 High Altitude Balloon Course}{Mechanical Lead}{January 2018 -- April 2018}
\begin{itemize}
    \item Used NX and CNC router to design and build structures for high altitude balloon payload
    \item Debugged and operated amateur radio for APRS tracking
    \item Developed prelaunch checklists and day-of-launch procedures
\end{itemize}
\worksubsection{SPACE583 Spacecraft Design Course}{Avionics Team Member}{January 2018 -- April 2018}
\begin{itemize}
    \item Designed electrical subsystems for Deep Space Gateway spacecraft tug
    \item Delivered computer vision prototypes to validate autonomous tracking and docking concepts
\end{itemize}
\worksubsection{AEROSP483 Spacecraft Design Course}{Avionics Co-Lead}{January 2017 -- April 2017}
\begin{itemize}
    \item Designed Deimos sampling mission
\end{itemize}
\worksubsection{AEROSP405 Design-Build-Test Course}{Instrumentation Lead}{September 2016 -- December 2016}
\begin{itemize}
    \item Designed data acquisition system around Arduino Nano for regenerative rocket nozzle testing
    \item Selected and calibrated sensors for testing
    \item Processed raw data and provided useful data to the team for analysis
\end{itemize}
\worksubsection{AEROSP205 Course}{Student}{September 2014 -- December 2014}
\begin{itemize}
    \item Used CATIA and Star-CCM+ to design hovercraft shell
    \item Manufactured hovercraft shell using CNC routing and composite layups
\end{itemize}
\worksubsection{U-M ARCLab}{Research Assistant}{September 2014 -- December 2014}
\worksubsection{ENGR100 Course}{Student}{January 2014 -- April 2014}
\begin{itemize}
    \item Designed envelope for student built blimp
    \item Managed mass budgets
\end{itemize}
\datedbfsubsection{Research Team}{September 2011 -- March 2013}
\worksubsubsection{Western Michigan University Innovation Center}{Research Assistant}{September 2012 -- March 2013}
\worksubsubsection{Pfizer R\&D}{Student Researcher}{September 2011 -- March 2012}

\section{Skills}
\basesubsection{Computer Skills}
\begin{itemize}
    \listitem{Programming}{C, C++, MATLAB, Python, Objective-C, Java, JavaScript, VBA, Subversion/Git, Docker}
    \listitem{System Design}{NX/Teamcenter, Creo/Windchill, SolidWorks, Simulink, CATIA, CircuitMaker}
    \listitem{System Analysis}{ANSYS, Multisim, Star-CCM+, FLUENT, Easy5, XFOIL, SimDesigner}
    \listitem{Operating Systems}{Linux, Windows, macOS}
    \listitem{General Software}{MS Office (Word, Excel, PowerPoint), MS Visio}
\end{itemize}
\basesubsection{Manufacturing Skills}
\begin{itemize}
    \listitem{Basic Tools}{Proficient with standard shop tools, laser cutter, 3D printer, composite manufacturing}
    \listitem{Machining}{Proficient in manual mill/manual lathe, familiar with CNC lathe/CNC router}
\end{itemize}
\basesubsection{Other Skills}
\begin{itemize}
    \item Amateur Radio Operator: Technician (KE8HMM)
\end{itemize}

\section{Honors/Achievements}
\begin{itemize}
    \item Winner, Region III Student Conference - Team Category
    \item Participant, 2018 AIAA Foundation International Student Conference - Team Category
    \item Sigma Gamma Tau Aerospace Honor Society Active Member (former Web Team member)
    \item Jean Fairfax Scholarship: merit based renewable scholarship awarded each semester
    \item National Merit Scholar
    \item Dean's List, Fall 2013
\end{itemize}

\section{Publications}
\begin{itemize}
    \item TODO Convectively Cooled paper
    \item TODO antibodies
\end{itemize}

\section{Languages}
\begin{itemize}
    \listitem{Professional Working Proficiency}{French, Tamil}
\end{itemize}

\end{document}
